...
- When the source is a `.md` file, the host MUST run a detect-only markdown-hygiene pre-pass (Phase 0) on the source before dispatching the compression agent. The Phase 0 verdict is passed to the agent via `--hygiene-verdict <path>` and included in the compression output. Phase 0 is detect-only by default; the host MAY pass `--fix` if the workflow requires a clean source before compression — document the rationale when doing so.
...
- Do not introduce abbreviations that could collide with other terms, or introduce the same abbreviation more than once per concept per file.
- Do not install or invoke external packages — the skill operates on the agent's own language understanding.
