...
- SA012 [WARN] line 315: `## Behavior` is immediately followed by `### Posting a Message` (line 317) with no content between.
  Note: Same pattern — the Behavior section heading has no framing prose before its first sub-section.
...
- SA035 [WARN] line 145: action stated before gate condition in R4.
  Note: "The post tool MUST create the recipient's inbox directory if it does not already exist" — the conditional guard ("if it does not already exist") follows the action. Preferred order: "If the recipient's inbox directory does not already exist, the post tool MUST create it before writing the message file."

- SA014 [SUGGEST] line 399: directive word "always" appears unemphasized in the Don'ts section.
  Note: "always invoke the `post` tool" — the word "always" carries directive weight but is in plain prose; consistent with RFC 2119 style elsewhere, this could be "MUST always invoke" or written as "invoke only via the `post` tool."
