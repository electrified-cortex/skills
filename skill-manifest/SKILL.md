...
4. Dispatch `hash-record-manifest` with file list and `repo_root`. Receive `manifest_hash`.
5. Probe `hash-record(manifest_hash, "skill-manifest")`.
   - Hit: return stored result with `"cached": true`. Done — no subagent needed.
     Cache key covers `skill_dir` direct files only. Stale `files` or `broken_refs` possible if transitive deps outside `skill_dir` changed since last scan. Callers can force fresh scan by touching any `skill_dir` file.
