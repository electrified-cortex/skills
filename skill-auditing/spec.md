...
- **MISS**: return token emitted when a cache record is found; verdict not re-computed.
- **skipped**: result value emitted when the record is a cache hit (MISS path); the audit was not re-run.
...
```text
Right:  PASS: <abs-path> | NEEDS_REVISION: <abs-path> | FAIL: <abs-path>
