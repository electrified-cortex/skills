...
Applies when a semantic advisory scan is needed on a single `.md` file. Covers SA001–SA038 only. Does not cover MD rule violations (handled by `lint`). Does not modify the target file. Independent of lint output — the host orchestrates sequencing.
