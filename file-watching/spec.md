...
- **changed** — one line emitted on stdout when the watched file's content has changed (write, append, atomic temp-then-rename save). Either fired immediately on the first change from idle (leading edge) or batched after a cooldown window if multiple changes piled up.
...
- **heartbeat** — one line emitted on stdout when the configured `-Heartbeat` window elapses with no `changed`. Proves the watcher is alive; lets callers distinguish "quiet" from "dead."
- **timeout** — one line emitted on stdout when the configured `-Timeout` window elapses with no `changed`; the script exits 0 immediately after.
...
- **Prefix** — `-Prefix <string>` / `--prefix <string>`. String prepended (followed by `": "`) to every emitted line. Default empty. Used by callers that consume multiple watch streams to disambiguate output.
- **gone** — emitted as a single output line when the watched file is detected to be deleted **while the script is running**. Followed immediately by exit 0. Provides a clean shutdown signal: a caller deletes the watched file to ask the watcher to die without process-killing it.
- **missing** — emitted as a single output line when the watched file does NOT exist **at script start**. Followed immediately by exit 0. The watcher is a pure consumer; missing file = nothing to watch = clean exit. Producers (or wrappers like `monitor.sh`) own file lifecycle.
...
- Optional output prefix (`-Prefix <string>` / `--prefix <string>`): when set, every emitted line is prepended with `<prefix>: `. Affects all four token types (changed, heartbeat, timeout, gone). Default empty (no prefix).
- **Delete-as-shutdown signal.** When the watched file is deleted while the script is running, emit `gone` (or `<prefix>: gone`) and exit 0. Brief verify (~200ms) is performed before exiting so atomic temp+rename saves do not trigger a false exit.
- **Pure consumer — no auto-create.** If the watched file does NOT exist at script start, emit `missing` (or `<prefix>: missing`) and exit 0. The watcher does NOT create the file or wait for it to appear. File lifecycle is owned by the producer/wrapper layer (e.g. `monitor.sh`, `post.sh`).
