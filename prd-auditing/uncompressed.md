...
`<prd_path>` — absolute path to the PRD file being audited (markdown).
...
Branch on the executor's return token:

- `CLEAN: <report_path>` — audit clean, no findings; done.
- `PASS: <report_path>` — non-blocking findings only; done.
- `NEEDS_REVISION: <report_path>` or `FAIL: <report_path>` — surface the
  report path and append:
...
- `prd-writing` — companion skill for authoring a PRD that passes this audit
